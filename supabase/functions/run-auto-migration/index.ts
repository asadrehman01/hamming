...
type SourcePreset = "generic" | "legacy_a" | "legacy_b";
...
const splitChunks = <T>(rows: T[], size = CHUNK_SIZE): T[][] => {
  const chunks: T[][] = [];
...
const parseCsvText = (csvText: string): Record<string, unknown>[] => {
...
const mapCustomerRowFromPreset = (
  ...
  if (sourcePreset === "legacy_a") {
    return {
      ...
      phone: String(
        normalizedSource.mobile_no || normalizedSource.phone || "",
      ).trim(),
      email: String(normalizedSource.email_id || normalizedSource.email || "")
        .trim()
        .toLowerCase(),
      membership_duration: String(
        normalizedSource.plan ||
          normalizedSource.membership_duration ||
          "1 MONTH",
      ).trim(),
      membership_start_date: String(
        normalizedSource.start_date ||
          normalizedSource.membership_start_date ||
          "",
      ).trim(),
      membership_end_date: String(
        normalizedSource.end_date || normalizedSource.membership_end_date || "",
      ).trim(),
    };
  }
  ...
  if (sourcePreset === "legacy_b") {
    return {
      first_name: String(
        normalizedSource.firstname || normalizedSource.first_name || "",
      ).trim(),
      last_name: String(
        normalizedSource.lastname || normalizedSource.last_name || "",
      ).trim(),
      phone: String(
        normalizedSource.phone_number || normalizedSource.phone || "",
      ).trim(),
      email: String(normalizedSource.email || "")
        .trim()
        .toLowerCase(),
      membership_duration: String(
        normalizedSource.plan_name ||
          normalizedSource.membership_duration ||
          "1 MONTH",
      ).trim(),
      membership_start_date: String(
  ...
  return {
    first_name: String(
      normalizedSource.first_name || normalizedSource.firstname || "",
    ).trim(),
    last_name: String(
      normalizedSource.last_name || normalizedSource.lastname || "",
    ).trim(),
    phone: String(
      normalizedSource.phone || normalizedSource.phone_number || "",
    ).trim(),
    email: String(normalizedSource.email || "")
      .trim()
      .toLowerCase(),
    ...
    membership_end_date: String(
      normalizedSource.membership_end_date || normalizedSource.end_date || "",
    ).trim(),
  };
};

const mapPaymentRowFromPreset = (
  normalizedSource: Record<string, unknown>,
  sourcePreset: SourcePreset,
): PaymentPayload => {
  if (sourcePreset === "legacy_a") {
    return {
      ...
      payment_mode: String(
        normalizedSource.mode || normalizedSource.payment_mode || "cash",
      ).toLowerCase(),
      sender_name: String(
        normalizedSource.customer_name || normalizedSource.sender_name || "",
      ).trim(),
      sender_account_name: String(
        normalizedSource.account_name ||
          normalizedSource.sender_account_name ||
          "",
      ).trim(),
      source_transaction_id: String(
        normalizedSource.transaction_id ||
          normalizedSource.source_transaction_id ||
          "",
      ).trim(),
      created_at: String(
        normalizedSource.payment_date ||
          normalizedSource.created_at ||
  ...
  if (sourcePreset === "legacy_b") {
    return {
      amount: parseFloat(String(normalizedSource.amount || 0)),
      status: String(
        normalizedSource.payment_status ||
      ...
      created_at: String(
        normalizedSource.created_at || new Date().toISOString(),
      ),
    };
  }
  ...
  return {
    ...
    sender_name: String(
      normalizedSource.sender_name || normalizedSource.customer_name || "",
    ).trim(),
    sender_account_name: String(
      normalizedSource.sender_account_name || "",
    ).trim(),
...
const insertRowsWithFallback = async (
  admin: ReturnType<typeof createClient>,
  {
    ...
    table: string;
    rows: Array<{ rowIndex: number; payload: Record<string, unknown> }>;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey || !serviceRoleKey) {
      throw new Error(
        "Missing Supabase environment variables for migration runner.",
      );
    }
    ...
    const sourcePreset = (
      ["generic", "legacy_a", "legacy_b"].includes(body?.sourcePreset)
        ? body.sourcePreset
        : "generic"
    ) as SourcePreset;
    ...
    if (!customerCsv && !paymentCsv) {
      return new Response(
        JSON.stringify({ error: "No CSV payload provided" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (customerCsv) {
      ...
      const normalizedRows: Array<{
        rowIndex: number;
        normalizedPayload: CustomerPayload;
        validationStatus: string;
      }> = [];
      ...
      // Build maps by querying for matches on incoming identifiers, batch by batch
      // to avoid loading all customers into memory (OOM risk for large gyms)
      const existingByPhone = new Map<string, string>();
      const existingByEmail = new Map<string, string>();
      const batchSize = 100;

      // Extract unique phones and emails from incoming rows
      const incomingPhones = new Set<string>();
      const incomingEmails = new Set<string>();
      validRows.forEach((row) => {
        const phone = normalizePhone((row.normalizedPayload as any).phone);
        const email = normalizeEmail((row.normalizedPayload as any).email);
        if (phone) incomingPhones.add(phone);
        if (email) incomingEmails.add(email);
      });

      // Query in batches for customers matching incoming identifiers
      const phoneBatches = Array.from(incomingPhones).reduce(
        (acc, phone, idx) => {
      ...
      const emailBatches = Array.from(incomingEmails).reduce(
        (acc, email, idx) => {
          if (idx % batchSize === 0) acc.push([]);
      ...
      // Fetch matching customers by phone
      ...
      // Fetch matching customers by email
      ...
      await logImportJob(admin, {
        ...
        normalizedRows: normalizedRows.map((row) => ({
          normalizedPayload: row.normalizedPayload as unknown as Record<
            string,
            unknown
          >,
    ...
    if (paymentCsv) {
      ...
      const normalizedRows: Array<{
        rowIndex: number;
        normalizedPayload: PaymentPayload;
        validationStatus: string;
      }> = [];
      const validationErrors: ValidationError[] = [];
      ...
      const { successCount, successfulRowIndices } =
        await insertRowsWithFallback(admin, {
          table: "payments",
          rows: rowsForInsert,
          rowErrors: dbErrors,
      ...
      importedRevenue = validRows
        .filter(
          (row) =>
            ...
            successfulRowIndexSet.has(row.rowIndex),
        )
        .reduce(
          (sum, row) =>
            sum + parseFloat(String(row.normalizedPayload.amount || 0)),
          0,
        );
    ...
    if (notifyEmail) {
      ...
      try {
        ...
        if (!notifyResponse.ok) {
          const notifyError = await notifyResponse
            .text()
